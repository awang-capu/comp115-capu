...
# Modify the following expression:
#     (marks[0] + marks[1]) / 2
# so that it calculates the correct average of all marks.
# ---------------------------------------------------------

marks = [80.5, 76.5, 85.3, 79.8]  # A list of a student's first-term course marks
mark_average = sum(marks) / len(marks)  # Modify this to get the correct average
...
day_today = int(input("Enter today's number: "))
days_trip = int(input("How many days is your trip? "))
print(f"Your trip starts on day {day_today}, lasts days {days_trip}. You are back on day { (day_today + days_trip) % 7 }. ")
